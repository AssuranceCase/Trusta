...
class DataService:
    ...
    # 通过结点名称，创建一个默认结点
    # force==True表示强制创建一个结点，不论结点名称是否存在
    ...
    def setNodeColor(self, ID, is_normal):
        for node in self.nodeDataArray:
            if node["key"] == ID:
                if is_normal:
                    node['frameColor'] = '#C4ECFF' # 蓝色
                else:
                    node['frameColor'] = '#FFFF66' # 黄色
    ...
    def getAllChildrenColor(self, ID):
        ...
        if list_child_color.count('#FFFF66') == len(list_childrenID) and len(list_childrenID) != 0: # 黄色
            return 'all_#FFFF66'
        else:
            return ''
    ...
    def ConstraintProve(self, parentID):
        ...
        try:
            ...
            elif reason in HAN_EN.gets('LogicalRelations'): # prolog类型
                check_obj = prologcheck.PrologCheck()
            elif reason in HAN_EN.gets('Quantified'): # D-S理论扩展
    ...
    def tintagePathParent(self, node):
        ...
        list_IDs = [] # 保存所有错误结点的路径父结点ID
        self.getPathParentIDs(ID, list_IDs) # 保存错误结点的父结点ID
        # 所有父结点标记为黄色(到"或"截至)
        stop_idx = len(list_IDs) - 1 # 默认到根结点
    ...
    def loadDot(self, dot_path, encoding='utf-8'):
        ...
        self.setAllNodeWidth(300) # 结点默认宽度
        # 写入show_data.js文件
        jstrans.py2js(self.getStrData())
    ...
    def functionAnalysisLLM(self, rootID, level, temperature):
        ...
        list_descendant_ids = list_descendant_ids[::-1] # 从前序遍历 -> 先访问子结点，再访问父节点
        ...
        for i, ID in enumerate(list_descendant_ids):
            ...
            self.setNodeName(node, new_name=result_info['function_goal'].strip('\n'))
    ...
    def checkAllExpr(self, proveRootID):
        ...
        for node in self.nodeDataArray:
            ...
            if ret:
                dict_err_info['结点编号为 {} 子树的错误信息'.format(ID)] = ret
                self.tintagePathParent(node) # 路径上的父节点染色

        # 所有兄弟结点是黄色，上层继续标黄,(主要针对”或“)
    ...
    def parse_prolog_all(self, src, quotesSwitch):
        # pattern = r'"?.*?"?\s*:-\s*("?.*?"?,\s*)*?("?.*?"?\.)' # 引号可选
        if quotesSwitch in HAN_EN.gets('Yes'):
            pattern = r'".*?"\s*:-\s*(".*?",\s*)*?(".*?"\.)' # 匹配 "A" :- "B C", D.
        else:
            pattern = r'.*?\s*:-\s*(.*?,\s*)*?(.*?\.)' # 匹配 A :- B, C, D.
    ...
    def yamlImport(self, dstFilePath, encoding):
        ...
        self.setAllNodeWidth(300) # 结点默认宽度,更新数据
        return ''
        ...
        for node in self.nodeDataArray:
            ...
            if addition:
                ...
                try:
                    addition = json.loads(addition)
                    if "背景" in addition:
                        addition['Context'] = addition['背景']
                        del addition['背景']
                    if "策略" in addition:
                        addition['Strategy'] = addition['策略']
                        del addition['策略']
                    if "假设" in addition:
                        addition['Assumption'] = addition['假设']
                        del addition['假设']
                    if "理由" in addition:
                        addition['Justification'] = addition['理由']
                        del addition['理由']
                except:
                    addition = {"错误": "ADDITION不是一个正确的JSON"}
            else:
                addition = {}
            ...
            for type, cont in addition.items():
                ...
                if self.isAdditionKey(type):
                    ...
                    add_key = '{}{}{}'.format(type[0], node['key'],
                                              type[-1] if len(type) > 1 and type[-1].isnumeric() else '') # 取最后一位数字，非数字则不取
                    gsn[add_key] = {
    ...
    def addToCompareData(self, proveRootID, proveLevel, domain, creator):
        ...
        for node in self.nodeDataArray:
            ...
            if len(dict_childrenName_nodetype) == 0 or is_all_solution:
                continue # 最后一层Solution不考虑，只统计 argument
